...
def create_training_step(
    model, optimizer, calc_loss_contribution
):
    ...
    def training_step(
        ...
        def calc_loss(model_params):
            ...
            def invoke_loss_contribution(
                ...
                ## for EAN dataset, do not evaluate forces
                # pred_energy = model.apply(
                #     model_params, positions, types, cell,
    ...
    def training_epoch(optimizer_state, model_params):
        ...
        with ExitStack() as with_stack:
            ...
            for (positions_batch,
                ...
                if log_wandb:
                    wandb.log({"loss": loss.copy(), 
                        "step":training_epoch.epoch+1}, commit=False)
                if progress_bar:
            ...
            if log_wandb:
                ...
                nconfigs = positions_train.shape[0]
                time_per_config = round(elapsed / nconfigs, 6)
                wandb.log({"time/sec_per_config": time_per_config, 
                    "time/it_per_s": 1/time_per_config,
                    "time/mins_per_epoch": elapsed/60}, commit=False)
...
def _ermse_map_function(pred_energy, pred_forces, obs_energy, obs_forces):
    return ((obs_energy - pred_energy) / pred_forces.shape[0] )**2
